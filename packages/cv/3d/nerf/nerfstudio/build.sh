...
uv pip install --extra-index-url https://pypi.org/simple scikit-build-core ninja # ninja is often needed too

cd /opt/nerfstudio
uv pip install --extra-index-url https://pypi.org/simple --reinstall blinker
sed -i 's/==/>=/g' pyproject.toml
uv pip install --extra-index-url https://pypi.org/simple cmake open3d
uv pip install --extra-index-url https://pypi.org/simple --reinstall blinker
...
cd /
uv pip install --extra-index-url https://pypi.org/simple manifold3d vhacdx openexr
uv pip install --extra-index-url https://pypi.org/simple --reinstall blinker
uv pip install --extra-index-url https://pypi.org/simple /opt/nerfstudio/wheels/nerfstudio*.whl

ns-install-cli --mode install

cd /opt/nerfstudio
uv pip install --extra-index-url https://pypi.org/simple -U --force-reinstall opencv-contrib-python
