...
def cuda_stack_args():
    ...
    if IS_TEGRA:
        if L4T_VERSION.major >= 39:  # JetPack 6 on Orin
            ...
            if CUDA_VERSION >= Version("13.2"):
                ...
                cudnn_url = f"https://developer.download.nvidia.com/compute/cudnn/9.21.1/local_installers/cudnn-local-repo-{distro}-9.21.1_1.0-1_arm64.deb"
                cudnn_packages = (
                    "libcudnn9-cuda-13 libcudnn9-dev-cuda-13 libcudnn9-samples"
                )
                tensorrt_ver = "10.16.1"
                tensorrt_url = f"{tensorrt_base_url}/10.16.1/tars/TensorRT-10.16.1.11.Linux.aarch64-gnu.cuda-13.2.tar.gz"
                nccl_ver = "2.29.7"
                ...
                cudnn_packages = (
                    "libcudnn9-cuda-12 libcudnn9-dev-cuda-12 libcudnn9-samples"
                )
                tensorrt_ver = "10.13"
    ...
    return {
        ...
        "TENSORRT_DEB": os.path.basename(tensorrt_url).split("_")[0]
        if tensorrt_url.endswith(".deb")
        else "",
        ...
        "CUDA_INSTALLED_VERSION": int(
            str(CUDA_VERSION.major) + str(CUDA_VERSION.minor)
        ),
...
def cuda_stack(name, with_tensorrt=False, minimal=False, requires=None):
    ...
    pkg["depends"] = ["cuda", "python"]  # cudastack depends ON cuda, doesn't replace it
...
if IS_TEGRA and IS_CONFIG:
    package = [
        ...
        cuda_stack(
            "cudastack:minimal", with_tensorrt=False, minimal=True, requires=">=35"
        ),
        # Standard: + TensorRT and all CUDA libraries
        cuda_stack(
            "cudastack:standard", with_tensorrt=True, minimal=False, requires=">=35"
        ),
    ]
    ...
    package = [
        cuda_stack(
            "cudastack:minimal", with_tensorrt=False, minimal=True, requires="aarch64"
        ),
        cuda_stack(
            "cudastack:standard", with_tensorrt=True, minimal=False, requires="aarch64"
        ),
    ]
elif IS_CONFIG:
    # x86_64
    package = [
        cuda_stack(
            "cudastack:minimal", with_tensorrt=False, minimal=True, requires="x86_64"
        ),
        cuda_stack(
            "cudastack:standard", with_tensorrt=True, minimal=False, requires="x86_64"
        ),
    ]
