...
uv pip install --extra-index-url https://pypi.org/simple nanobind wheel 'setuptools>=80.9.0' 'setuptools-scm>=9.2.0' twine
...
uv pip install --extra-index-url https://pypi.org/simple /opt/unsloth/wheels/unsloth*.whl

sed -i 's/==/>=/g' pyproject.toml && \
sed -i 's/~=/>=/g' pyproject.toml

cd /opt/unsloth
uv pip install --extra-index-url https://pypi.org/simple compressed-tensors 'unsloth_zoo>=2026.4.3' cut_cross_entropy
