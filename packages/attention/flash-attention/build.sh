...
uv pip install --extra-index-url https://pypi.org/simple /opt/flash-attention/wheels/flash_attn-*.whl
...
uv build --wheel . -v --no-build-isolation --out-dir /opt/flash-attention/wheels
uv pip install --extra-index-url https://pypi.org/simple /opt/flash-attention/wheels/flash_attn_*.whl || echo "Failed to install FlashAttention4(a CuTeDSL-based implementation for Hopper and Blackwell GPUs)"
